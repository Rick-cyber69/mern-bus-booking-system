...
const seedDatabase = async () => {
  try {
    ...
    console.log('----------------------------------------------------');
    console.log('EXPANDED INDIAN BUS NETWORK SEEDING COMPLETED SUCCESSFULLY!');
    const allCities = Array.from(new Set(routeDefinitions.flatMap(r => [r.originCity, r.destinationCity]))).sort();
    console.log(`Total Cities in Dropdown (${allCities.length}):`);
